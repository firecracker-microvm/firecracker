...
// Criteria for the present list of registers: only R/W registers, optional
// registers are not saved. GICC_NSAPR are not saved since they are only present
// in GICv2 implementations that include the GIC security extensions so it might
