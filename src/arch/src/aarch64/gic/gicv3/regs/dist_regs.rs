...
// implementation specific registers are not saved. GICD_CPENDSGIR and
// GICD_SPENDSGIR are not saved since these registers are not used when affinity
// routing is enabled. Affinity routing GICv3 is enabled by default unless
// Firecracker clears the ICD_CTLR.ARE bit which it does not do. NOTICE: Any
// changes to this structure require a snapshot version bump.
