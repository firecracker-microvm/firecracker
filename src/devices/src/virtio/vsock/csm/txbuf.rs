...
impl TxBuf {
    ...
    pub fn push(&mut self, src: &[u8]) -> Result<()> {
        ...
        // Pushing a slice to this buffer can take either one or two slice copies: - one
        // copy, if the slice fits between `head_ofs` and `Self::SIZE`; or - two
        // copies, if the ring-buffer head wraps around.

        // First copy length: we can only go from the head offset up to the total buffer
        // size.
        let len = std::cmp::min(Self::SIZE - head_ofs, src.len());
        data[head_ofs..(head_ofs + len)].copy_from_slice(&src[..len]);

        // If the slice didn't fit, the buffer head will wrap around, and pushing
        // continues from the start of the buffer (`&self.data[0]`).
        if len < src.len() {
            data[..(src.len() - len)].copy_from_slice(&src[len..]);
        }

        // Either way, we've just pushed exactly `src.len()` bytes, so that's the amount
        // by which the (wrapping) buffer head needs to move forward.
        ...
        W: Write,
    {
        ...
        // - two writes, if the tail would wrap around: tail to slice end, then slice
        //   end to head.
        ...
        // It's safe to unwrap here, since we've already checked if the buffer was
        // empty.
        let data = self.data.as_ref().unwrap();

        // Issue the first write and absorb any `WouldBlock` error (we can just try
        // again later).
        let written = sink
            .write(&data[tail_ofs..(tail_ofs + len_to_write)])
            .map_err(Error::TxBufFlush)?;

        // Move the buffer tail ahead by the amount (of bytes) we were able to flush
        // out.
        self.tail += Wrapping(written as u32);

        // If we weren't able to flush out as much as we tried, there's no point in
        // attempting our second write.
        if written < len_to_write {
            return Ok(written);
        }

        // Attempt our second write. This will return immediately if a second write
        // isn't needed, since checking for an empty buffer is the first thing
        // we do in this function.
        //
        // Interesting corner case: if we've already written some data in the first
        // pass, and then the second write fails, we will consider the flush
        // action a success and return the number of bytes written in the first
        // pass.
