...
mod tests {
    ...
    fn test_vsock_bof() {
        ...
        test_ctx.mem = vm_memory::create_guest_memory(
            &[
                (None, GuestAddress(0), 8 * MIB),
                (None, GuestAddress((GAP_START_ADDR - MIB) as u64), MIB),
                (None, GuestAddress(FIRST_AFTER_GAP as u64), MIB),
            ],
            false,
        )
        .unwrap();
