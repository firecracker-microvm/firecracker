...
/// with the connection pool).  A desynchronized RX queue still holds
/// valid data, and the muxer will continue to pop packets from it. However,
/// when a desynchronized queue is drained, additional data may still be
/// available, so the muxer will have to perform a more costly walk of the
/// entire connection pool to find it.  This walk is performed here, as part of
/// building an RX queue from the connection pool. When an out-of-sync is
/// drained, the muxer will discard it, and attempt to rebuild a synced one.
...
impl MuxerRxQ {
    ...
    /// pool. Note: the resulting queue may still be desynchronized, if
    /// there are too many connections       that have pending RX data. In
    /// that case, the muxer will first drain this queue, and       then try
    /// again to build a synchronized one.
    ...
    /// queried for pending RX data later. Aside from this queue, there is
    /// no other storage for RSTs, so failing to push one means that we have
    /// to drop the packet.
    ///
    /// Returns:
    ...
    pub fn push(&mut self, rx: MuxerRx) -> bool {
        ...
        match rx {
            MuxerRx::RstPkt { .. } => {
                // If we just failed to push an RST packet, we'll look through the queue, trying
                // to find a connection key that we could evict. This way, the
                // queue does lose sync, but we don't drop any packets.
