...
///   * sector: an u64 value representing the offset where a read/write is to
///     occur.
///
...
impl RequestHeader {
    ...
    ///
    /// Virtio 1.0 specifies that the data is transmitted by the driver in
    /// little-endian format. Firecracker currently runs only on little
    /// endian platforms so we don't need to do an explicit little endian
    /// read as all reads are little endian by default. When running on a
    /// big endian platform, this code should not compile, and support
...
mod tests {
    ...
    // This can also be generated by a derive macro from `proptest_derive`, but the
    // crate is currently experimental.
    ...
    impl Arbitrary for RequestType {
        ...
        // Tuple union will hold the strategies that we use to generate the request
        // type. The first element is the weight of the strategy, the second is
        // a function that returns the strategy value.
    ...
    fn random_request_parse(
        ...
        // parsing the descriptor chain. Input properties are validated and commented
        // below.
        (
