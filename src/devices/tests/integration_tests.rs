...
pub type TestDevice = SerialWrapper<EventFdTrigger, SerialEventsWrapper, Box<Stdout>>;

fn create_serial(pipe: c_int) -> Arc<Mutex<TestDevice>> {
...
fn test_issue_serial_hangup_anon_pipe_while_registered_stdin() {
    ...
    TestDevice::init(serial.clone(), &mut event_manager);
...
fn test_issue_hangup() {
    ...
    assert_eq!(event_manager.wait(None), Ok(true));

    // Nothing can interrupt us.
    assert_eq!(event_manager.wait(Some(1)), Ok(false));
}
...
fn test_issue_serial_hangup_anon_pipe_while_unregistered_stdin() {
    ...
    // `EventSet::IN` was received on stdin. The event handling will consume
    // 64 bytes from stdin. The stdin monitoring is still armed.
    assert_eq!(event_manager.wait(Some(0)), Ok(true));
    ...
    // since it was not unregistered before, it will do a noop.
    {
        assert_eq!(event_manager.wait(), Ok(true));
        assert_eq!(event_manager.wait(), Ok(true));
    }
    ...
    assert_eq!(event_manager.wait(Some(0)), Ok(false));
