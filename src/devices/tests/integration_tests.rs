...
fn test_issue_serial_hangup_anon_pipe_while_registered_stdin() {
    ...
    // Register the reading end of the pipe to the event manager, to be processed
    // later on.
    ...
    // Read the 65th from the stdin through the kick stdin event triggered by 64th
    // of the serial FIFO read, or by the armed level-triggered stdin
    // monitoring. Either one of the events might be handled first. The handling
    // of the second event will find the stdin without any pending
    // bytes and will result in EWOULDBLOCK. Usually, EWOULDBLOCK will reregister
    // the stdin, but since it was not unregistered before, it will do a noop.
    ...
    // This cycle of epoll has two important events. First, the received HANGUP and
    // second the fact that the FIFO is full, so even if the stdin reached EOF,
    // there are still pending bytes to be read. We still unregister the stdin
    // and keep reading from it until we get all pending bytes.
    ev_count = event_manager.run().unwrap();
    assert_eq!(ev_count, 1);
    ...
    // Process the kick stdin event generated by the reading of the 64th byte of the
    // serial FIFO. This will consume some more bytes from the stdin while the
    // stdin is unregistered.
    ev_count = event_manager.run().unwrap();
    assert_eq!(ev_count, 1);

    // Two more bytes left. At the 2nd byte, another kick read stdin event is
    // generated, trying to fill again the serial FIFO with more bytes.
    for i in 0..2 {
        serial
            .lock()
            .unwrap()
    ...
    // This can be deduced by reading from a non-blocking fd and getting 0 bytes as
    // a result, instead of EWOUDBLOCK. We unregister the stdin and the kick
    // stdin read evt.
    ev_count = event_manager.run().unwrap();
    assert_eq!(ev_count, 1);
...
fn test_issue_serial_hangup_anon_pipe_while_unregistered_stdin() {
    ...
    // Serial FIFO is full, so silence the stdin. We do not need any other
    // interruptions until the serial FIFO is freed.
    ev_count = event_manager.run().unwrap();
    assert_eq!(ev_count, 1);

    // Close the writing end (this sends an HANG_UP to the reading end).
    // While the stdin is unregistered, this event is not caught by the event
    // manager.
    rc = unsafe { libc::close(fds[1]) };
    assert!(rc == 0);

    // This would be a blocking epoll_wait, since the buffer is full and stdin is
    // unregistered. There is no event that can break the epoll wait loop.
    ...
    // Process the kick stdin event generated by the reading of the 64th byte of the
    // serial FIFO. This will consume some more bytes from the stdin. Keep in
    // mind that the HANGUP event was lost and we do not know that the stdin
    // reached EOF.
    ev_count = event_manager.run().unwrap();
    assert_eq!(ev_count, 1);

    // Two more bytes left. At the 2nd byte, another kick read stdin event is
    // generated, trying to fill again the serial FIFO with more bytes. Keep in
    // mind that the HANGUP event was lost and we do not know that the stdin
    // reached EOF.
    for i in 0..2 {
        serial
