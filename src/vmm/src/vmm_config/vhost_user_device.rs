...
pub struct VhostUserDeviceConfig {
    ...
    /// Size of the configuration space to fetch from the backend, which has
    /// to answer with exactly this many bytes. Only the device type knows how
    /// large its configuration space is, so a frontend that is agnostic to the
    /// device type cannot work it out. Defaults to 256 if not specified.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub config_space_size: Option<u32>,
}
