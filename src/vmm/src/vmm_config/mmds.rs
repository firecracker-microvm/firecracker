...
pub enum MmdsConfigError {
    ...
    #[error("The list of network interface IDs that allow forwarding MMDS requests is \
    empty.")]
    ...
    #[error("The list of network interface IDs provided contains at least one ID that \
    does not correspond to any existing network interface.")]
