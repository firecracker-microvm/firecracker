...
impl MemoryHotplugConfig {
    ...
    pub fn validate(&self) -> Result<(), MemoryHotplugConfigError> {
        ...
        if self.block_size_mib < min_block_size_mib {
            return Err(MemoryHotplugConfigError::InvalidBlockSizeTooSmall(
                min_block_size_mib,
            ));
        }
        if !self.block_size_mib.is_power_of_two() {
            return Err(MemoryHotplugConfigError::InvalidBlockSizePowerOfTwo);
        }

        let min_slot_size_mib = bytes_to_mib(VIRTIO_MEM_DEFAULT_SLOT_SIZE);
        if self.slot_size_mib < min_slot_size_mib {
            return Err(MemoryHotplugConfigError::InvalidSlotSizeTooSmall(
                min_slot_size_mib,
            ));
        }
        if !self.slot_size_mib.is_multiple_of(self.block_size_mib) {
            return Err(MemoryHotplugConfigError::InvalidSlotSizeMultiple(
                self.block_size_mib,
            ));
        }

        if self.total_size_mib < self.slot_size_mib {
            return Err(MemoryHotplugConfigError::InvalidTotalSizeTooSmall(
                self.slot_size_mib,
            ));
        }
        if !self.total_size_mib.is_multiple_of(self.slot_size_mib) {
            return Err(MemoryHotplugConfigError::InvalidTotalSizeMultiple(
                self.slot_size_mib,
            ));
        }
...
mod tests {
    ...
    fn test_block_size_too_small() {
        ...
        match config.validate() {
            Err(MemoryHotplugConfigError::InvalidBlockSizeTooSmall(min)) => assert_eq!(min, 2),
    ...
    fn test_block_size_not_power_of_two() {
        ...
        match config.validate() {
            Err(MemoryHotplugConfigError::InvalidBlockSizePowerOfTwo) => {}
    ...
    fn test_slot_size_too_small() {
        ...
        match config.validate() {
            Err(MemoryHotplugConfigError::InvalidSlotSizeTooSmall(min)) => assert_eq!(min, 128),
    ...
    fn test_slot_size_not_multiple_of_block_size() {
        ...
        match config.validate() {
            Err(MemoryHotplugConfigError::InvalidSlotSizeMultiple(block_size)) => {
    ...
    fn test_total_size_too_small() {
        ...
        match config.validate() {
            Err(MemoryHotplugConfigError::InvalidTotalSizeTooSmall(slot_size)) => {
    ...
    fn test_total_size_not_multiple_of_slot_size() {
        ...
        match config.validate() {
            Err(MemoryHotplugConfigError::InvalidTotalSizeMultiple(slot_size)) => {
                assert_eq!(slot_size, 128)
            }
