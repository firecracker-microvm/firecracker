...
use vm_memory::{
    GuestMemoryBackend, GuestMemoryError, GuestMemoryRegionBytes, ReadVolatile,
...
use crate::vstate::vm::{KvmVm, VmError, kvm_userspace_memory_region2};
...
/// Calculate the size of the bitmap in bytes to represent memory of size mem_size (in bytes)
pub fn bitmap_size(mem_size: u64) -> u64 {
    mem_size / host_page_size() as u64 / u8::BITS as u64
}
...
impl GuestRegionMmapExt {
    ...
    pub(crate) fn mem_slot(&self, slot: u32) -> GuestMemorySlot<'_> {
        ...
        GuestMemorySlot {
            ...
            userfault_bitmap: self
                .userfault_bitmap
                .map(|v| v.wrapping_add(bitmap_size(offset))),
        }
    }
...
mod tests {
    ...
    fn test_snapshot_file_success() {
        for dirty_page_tracking in [true, false] {
            ...
            let guest_regions = file_private(
                Arc::new(file),
                regions.into_iter(),
                dirty_page_tracking,
                HugePageConfig::None,
    ...
    fn test_check_range_plugged() {
        ...
        let ext = GuestRegionMmapExt::from_state(region, &state, 0, None).unwrap();
