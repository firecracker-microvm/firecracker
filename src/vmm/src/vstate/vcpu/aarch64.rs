...
impl KvmVcpu {
    ...
    pub fn save_state(&self) -> Result<VcpuState> {
        ...
        crate::arch::regs::save_core_registers(&self.fd, &mut state.regs).map_err(Error::SaveState)?;

        crate::arch::regs::save_system_registers(&self.fd, &mut state.regs).map_err(Error::SaveState)?;
...
mod tests {
    ...
    fn test_configure_vcpu() {
        ...
        assert!(vcpu
            .configure(&vm_mem, GuestAddress(crate::arch::get_kernel_start()),)
            .is_ok());
