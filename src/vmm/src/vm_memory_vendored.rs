...
use vm_memory::{Error, GuestAddress, GuestMemory, GuestMemoryRegion, GuestUsize};
...
impl<R: GuestMemoryRegion> GuestRegionCollection<R> {
    ...
    /// Remove a region from the [`GuestRegionCollection`] object and return a new
    /// `GuestRegionCollection` on success, together with the removed region.
    ///
    /// # Arguments
    ...
    pub fn remove_region(
        ...
        base: GuestAddress,
        size: GuestUsize,
    ) -> std::result::Result<(GuestRegionCollection<R>, Arc<R>), Error> {
