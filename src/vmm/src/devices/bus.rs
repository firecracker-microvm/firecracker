...
impl BusDevice {
    pub fn i8042_device_ref(&self) -> Option<&Arc<Mutex<I8042Device>>> {
        match self {
    ...
    #[cfg(target_arch = "aarch64")]
    pub fn rtc_device_ref(&self) -> Option<&Arc<Mutex<RTCDevice>>> {
        match self {
            Self::RTCDevice(x) => Some(x),
            _ => None,
        }
    }
    pub fn boot_timer_ref(&self) -> Option<&Arc<Mutex<BootTimer>>> {
        match self {
            Self::BootTimer(x) => Some(x),
            _ => None,
        }
    }
    pub fn mmio_transport_ref(&self) -> Option<&Arc<Mutex<MmioTransport>>> {
        println!("{:?}", self);
        match self {
    ...
    pub fn serial_ref(&self) -> Option<&Arc<Mutex<SerialDevice<std::io::Stdin>>>> {
        match self {
