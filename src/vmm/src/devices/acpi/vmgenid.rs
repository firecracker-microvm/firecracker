...
impl VmGenId {
    ...
    pub fn from_parts(guest_address: GuestAddress, gsi: u32) -> Self {
        ...
        let interrupt_evt = EventFdTrigger::new(
            EventFd::new(libc::EFD_NONBLOCK)
                .inspect_err(|err| {
                    error!("vmgenid: Could not create EventFd for VMGenID device: {err}")
                })
                .unwrap(),
        );
    ...
    pub fn new(resource_allocator: &mut ResourceAllocator) -> Self {
        let gsi = resource_allocator
            ...
            .inspect_err(|err| error!("vmgenid: Could not allocate GSI for VMGenID: {err}"))
            .unwrap();
        ...
        let addr = resource_allocator
            ...
            .inspect_err(|err| error!("vmgenid: Could not allocate guest RAM for VMGenID: {err}"))
            .unwrap();
