...
impl PL061Device {
    ...
    pub fn from_state(state: &PL061State) -> Result<Self, PL061Error> {
        ...
        device.refresh_masked_interrupt(true)?;
        Ok(device)
    }
    ...
    pub fn trigger_power_button(&mut self) -> Result<(), PL061Error> {
        ...
        self.set_input_level(POWER_BUTTON_PIN, true)
    }
    ...
    pub fn bus_read(&mut self, offset: u64, data: &mut [u8]) {
        ...
        if let Some(value) = self.read_reg(offset) {
            ...
            // The guest has now seen the lines selected by this access, so any that the host
            // was holding asserted can be released, latching the falling edge.
            if offset < PL061_DATA_REG_END
                && let Err(err) = self.release_observed_lines(data_mask(offset))
            {
    ...
    /// Releases the held input lines covered by `mask` once the guest has read them.
    fn release_observed_lines(&mut self, mask: u8) -> Result<(), PL061Error> {
        let released = self.state.auto_release & mask;
        if released == 0 {
            return Ok(());
        }
...
mod tests {
    ...
    fn test_pl061_level_sensitive_interrupt_follows_the_level() {
        ...
        device.set_input_level(2, false).unwrap();
        assert_eq!(read_u32(&mut device, PL061_RIS), 0);
        assert_eq!(read_u32(&mut device, PL061_MIS), 0);
    }
