            ...
            match bar.region_type() {
                PciBarRegionType::IoRegion => {
                    ...
                    io_bus
                        ...
                        .unwrap();
                    error!("cannot register bus mappings {:x} {:x} IO", bar.addr(), bar.size());
                }
...
impl PciConfigIo {
    ...
    pub fn config_space_read(&self) -> u32 {
        ...
        self.pci_bus
            ...
            .map_or(0xffff_ffff, |d| {
                d.lock().unwrap().pci_device_mut().unwrap().read_config_register(register)
            })
    }
    ...
    pub fn config_space_write(&mut self, offset: u64, data: &[u8]) -> Option<Arc<Barrier>> {
        ...
        if let Some(d) = pci_bus.devices.get(&(device as u32)) {
            ...
            if let Some(params) = device.pci_device_mut().unwrap().detect_bar_reprogramming(register, data) {
            ...
            device.pci_device_mut().unwrap().write_config_register(register, offset, data)
        } else {
            None
        }
    }
