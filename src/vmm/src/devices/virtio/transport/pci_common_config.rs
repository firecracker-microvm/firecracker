...
impl VirtioPciCommonConfig {
    ...
    pub fn write(
        ...
        match data.len() {
            ...
            2 => self.write_common_config_word(offset, LittleEndian::read_u16(data), device.lock().unwrap().queues_mut()),
            4 => {
                self.write_common_config_dword(offset, LittleEndian::read_u32(data), device)
            }
            8 => self.write_common_config_qword(offset, LittleEndian::read_u64(data), device.lock().unwrap().queues_mut()),
    ...
    fn write_common_config_dword(
        ...
        match offset {
            ...
            0x20 => self.with_queue_mut(locked_device.queues_mut(), |q| lo(&mut q.desc_table_address, value)),
            0x24 => self.with_queue_mut(locked_device.queues_mut(), |q| hi(&mut q.desc_table_address, value)),
            0x28 => self.with_queue_mut(locked_device.queues_mut(), |q| lo(&mut q.avail_ring_address, value)),
            0x2c => self.with_queue_mut(locked_device.queues_mut(), |q| hi(&mut q.avail_ring_address, value)),
            0x30 => self.with_queue_mut(locked_device.queues_mut(), |q| lo(&mut q.used_ring_address,  value)),
            0x34 => self.with_queue_mut(locked_device.queues_mut(), |q| hi(&mut q.used_ring_address,  value)),
            _ => {
