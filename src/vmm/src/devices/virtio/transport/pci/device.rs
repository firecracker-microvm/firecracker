...
use pci::{
    ...
    PciBarRegionType, PciCapability, PciCapabilityId, PciClassCode, PciConfiguration,
        ...
        VirtioPciDeviceState {
            ...
            queues: self
                ...
                .map(|q| QueueState {
                    ...
                    size: q.size,
                    ready: q.ready,
                    desc_table: q.desc_table_address.0,
                    avail_ring: q.avail_ring_address.0,
                    used_ring: q.used_ring_address.0,
                })
                .collect(),
            ...
            msix_state: self
                ...
                .expect("Poisoned lock")
                .state(),
            msi_vector_group: self.interrupt_source_group.state(),
...
impl VirtioInterrupt for VirtioInterruptMsix {
    ...
    fn status(&self) -> Arc<std::sync::atomic::AtomicU32> {
