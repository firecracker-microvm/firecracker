...
use super::device::{ActiveBlock, BlockResources, BlockRuntimeState, DiskProperties};
...
impl Persist<'_> for VirtioBlock {
    ...
    fn save(&self) -> Self::State {
        let virtio_state =
            if let BlockRuntimeState::Active(ActiveBlock::Threaded(active)) = &self.state {
                VirtioDeviceState {
    ...
    fn restore(
        ...
        let queue = state
            ...
            .into_iter()
            .next()
            .ok_or(VirtioBlockError::Persist(PersistError::InvalidInput))?;
        ...
        Ok(VirtioBlock {
            ...
            state: BlockRuntimeState::Configuring(vec![resources], Vec::new()),
