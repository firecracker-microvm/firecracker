...
use crate::vmm_config::drive::{BlockDeviceConfig, default_num_queues};
...
impl VirtioBlockConfig {
    pub(crate) fn validate_queue_count(&self) -> Result<(), VirtioBlockError> {
        let max_queues = u16::from(MAX_SUPPORTED_VCPUS);
        if !(1..=max_queues).contains(&self.num_queues) {
            return Err(VirtioBlockError::InvalidQueueCount(
                self.num_queues,
                max_queues,
            ));
        }
        if self.num_queues > 1 && !self.threaded {
...
pub struct VirtioBlock {
    ...
    pub(crate) state: BlockRuntimeState,
...
impl BlockResources {
    /// Drain pending I/O and clear engine throttling
    fn reset(&mut self) {
        self.disk
            .file_engine
            .drain(true)
            .expect("Failed to reset block IO engine");
...
impl VirtioBlock {
    ...
    pub fn new(mut config: VirtioBlockConfig) -> Result<VirtioBlock, VirtioBlockError> {
        ...
        config.blk_size = Some(config_space.blk_size);
        config.topology = Some(config_space.topology);
    ...
    pub(crate) fn resources(&self) -> &[BlockResources] {
        match &self.state {
            ...
            BlockRuntimeState::Active(ActiveBlock::Inline(worker)) => {
                std::slice::from_ref(&worker.resources)
            }
    ...
    pub(crate) fn resources_mut(&mut self) -> &mut [BlockResources] {
        match &mut self.state {
            BlockRuntimeState::Configuring(resources, _) => resources,
            BlockRuntimeState::Active(ActiveBlock::Inline(worker)) => {
                std::slice::from_mut(&mut worker.resources)
            }
            BlockRuntimeState::Active(ActiveBlock::Threaded(_)) => {
                unreachable!("worker thread owns the runtime resources")
            }
    ...
    pub(crate) fn process_queue_event(&mut self) {
        if let BlockRuntimeState::Active(ActiveBlock::Inline(worker)) = &mut self.state {
    ...
    pub(crate) fn process_rate_limiter_event(&mut self) {
        ...
        if self.lock_rate_limiter().event_handler().is_err() {
            return;
        }
    ...
    pub fn update_rate_limiter(&mut self, bytes: BucketUpdate, ops: BucketUpdate) {
        let mut rate_limiter = self
            ...
            .lock()
            .expect("Poisoned block rate limiter lock");
        rate_limiter.update_buckets(bytes, ops);
        self.config.rate_limiter = RateLimiterConfig::from(&*rate_limiter).into_option();
    }
    ...
    pub(crate) fn spawn_worker(
        ...
        if let BlockRuntimeState::Configuring(resources, worker_handles) = &mut self.state {
            ...
            for resource in resources {
                ...
                let name = format!("fc_{}_q{}", self.config.drive_id, resource.queue_idx);
...
impl VirtioDevice for VirtioBlock {
    ...
    fn queue_config_mut(&mut self, index: usize) -> Option<&mut QueueConfig> {
        match &mut self.state {
            ...
            BlockRuntimeState::Active(ActiveBlock::Threaded(active)) => {
                active.queue_config.get_mut(index)
            }
            BlockRuntimeState::Placeholder => unreachable!("not a runtime state"),
        }
    }
    ...
    fn queue_event(&self, index: usize) -> Option<&EventFd> {
        match &self.state {
            ...
            BlockRuntimeState::Active(ActiveBlock::Threaded(active)) => active
                .worker_handles
                .get(index)
    ...
    fn interrupt_trigger(&self) -> &dyn VirtioInterrupt {
        match &self.state {
            ...
            BlockRuntimeState::Active(ActiveBlock::Threaded(active)) => active.interrupt.deref(),
            BlockRuntimeState::Configuring(_, _) => panic!("Device is not initialized"),
            BlockRuntimeState::Placeholder => unreachable!("not a runtime state"),
        }
    }
    ...
    fn activate(
        ...
        let BlockRuntimeState::Configuring(resources, _) = &mut self.state else {
        ...
        if worker_handles.is_empty() {
            ...
            self.state = BlockRuntimeState::Active(ActiveBlock::Inline(BlockWorker {
            ...
            let queue_config = resources.iter().map(|r| r.queue.config.clone()).collect();
            ...
            self.state = BlockRuntimeState::Active(ActiveBlock::Threaded(ThreadedActive {
                worker_handles,
                interrupt,
                queue_config,
            }));
    ...
    fn is_activated(&self) -> bool {
        matches!(&self.state, BlockRuntimeState::Active(_))
    }
...
impl Drop for VirtioBlock {
    fn drop(&mut self) {
        ...
        match std::mem::replace(&mut self.state, BlockRuntimeState::Placeholder) {
            BlockRuntimeState::Active(ActiveBlock::Inline(mut worker)) => match flush_mode {
                FlushMode::Drain => worker.drain(true),
                FlushMode::DrainAndFlush => worker.drain_and_flush(true),
            },
            BlockRuntimeState::Active(ActiveBlock::Threaded(active)) => {
                for handle in active.worker_handles {
                    handle.finish(flush_mode);
                }
            }
                ...
                for resource in &mut resources {
                    match flush_mode {
                        ...
                        FlushMode::DrainAndFlush => {
                            if let Err(err) = resource.disk.file_engine.drain_and_flush(true) {
...
mod tests {
    ...
    fn test_bandwidth_rate_limiter() {
        for engine in [FileEngineType::Sync, FileEngineType::Async] {
            ...
            // Following write procedure should fail because of bandwidth rate limiting.
            {
                ...
                assert!(block.lock_rate_limiter().is_blocked());
    ...
    fn test_validate_queue_count() {
        ...
        let path = f.as_path().to_str().unwrap().to_string();
        let max_queues = u16::from(MAX_SUPPORTED_VCPUS);
        let mut config = default_config(path.clone(), FileEngineType::Sync);
        config.threaded = true;
        config.num_queues = 0;
        assert!(matches!(
            VirtioBlock::new(config),
            Err(VirtioBlockError::InvalidQueueCount(0, max)) if max == max_queues
        ));

        let too_many = max_queues + 1;
        let mut config = default_config(path.clone(), FileEngineType::Sync);
        config.threaded = true;
        config.num_queues = too_many;
        assert!(matches!(
            VirtioBlock::new(config),
            Err(VirtioBlockError::InvalidQueueCount(n, max))
                if n == too_many && max == max_queues
        ));

        // Several queues need a worker thread, both at the device and at the API conversion.
        let mut config = default_config(path.clone(), FileEngineType::Sync);
        config.num_queues = 2;
        assert!(matches!(
            VirtioBlock::new(config),
