...
impl VirtioBlock {
    ...
    pub(crate) fn resources(&self) -> &BlockResources {
        match &self.state {
            ...
            BlockRuntimeState::Active(ActiveBlock::Inline(worker)) => &worker.resources,
    ...
    pub(crate) fn resources_mut(&mut self) -> &mut BlockResources {
        match &mut self.state {
            BlockRuntimeState::Configuring(resources, _) => resources,
            BlockRuntimeState::Active(ActiveBlock::Inline(worker)) => &mut worker.resources,
            BlockRuntimeState::Active(ActiveBlock::Threaded(_)) => {
                unreachable!("worker thread owns the runtime resources")
            }
            BlockRuntimeState::Placeholder => unreachable!("not a runtime state"),
...
impl VirtioDevice for VirtioBlock {
    ...
    fn activate(
        ...
        self.state = if let Some(worker_handle) = worker_handle {
            ...
            BlockRuntimeState::Active(ActiveBlock::Threaded(ThreadedActive {
                worker_handle,
                interrupt,
    ...
    fn _reset(&mut self) -> bool {
        ...
        let (mut resources, worker_handle) = match state {
            ...
            BlockRuntimeState::Active(ActiveBlock::Inline(worker)) => (worker.resources, None),
