...
mod tests {
    ...
    fn test_event_fail_descriptor_chain_too_short() {
        ...
        let queue_event_count = mem_dev.metrics.queue_event_count.count();
        let queue_event_fails = mem_dev.metrics.queue_event_fails.count();

        th.add_desc_chain(MEM_QUEUE, 0, &[(0, REQ_SIZE, 0)]);
        assert_eq!(th.emulate_for_msec(100).unwrap(), 1);

        assert_eq!(
            mem_dev.metrics.queue_event_count.count(),
            queue_event_count + 1
        );
        assert_eq!(
            mem_dev.metrics.queue_event_fails.count(),
            queue_event_fails + 1
        );
    }
    ...
    fn test_event_fail_descriptor_length_too_small() {
        ...
        assert_eq!(
            mem_def.metrics.queue_event_fails.count(),
            queue_event_fails + 1
        );
    }
    ...
    fn test_event_fail_unexpected_writeonly_descriptor() {
        ...
        let queue_event_count = self.metrics.queue_event_count.count();
        let queue_event_fails = self.metrics.queue_event_fails.count();

        th.add_desc_chain(MEM_QUEUE, 0, &[(0, REQ_SIZE, VIRTQ_DESC_F_WRITE)]);
        assert_eq!(th.emulate_for_msec(100).unwrap(), 1);

        assert_eq!(
            self.metrics.queue_event_count.count(),
            queue_event_count + 1
        );
        assert_eq!(
            self.metrics.queue_event_fails.count(),
            queue_event_fails + 1
        );
    }
    ...
    fn test_plug_request_success() {
        ...
        let queue_event_count = self.metrics.queue_event_count.count();
        let queue_event_fails = self.metrics.queue_event_fails.count();
        let plug_count = self.metrics.plug_count.count();
        let plug_bytes = self.metrics.plug_bytes.count();
        let plug_fails = self.metrics.plug_fails.count();
        ...
        assert_eq!(self.metrics.queue_event_fails.count(), queue_event_fails);
        assert_eq!(self.metrics.plug_count.count(), plug_count + 1);
        assert_eq!(self.metrics.plug_bytes.count(), plug_bytes + (2 << 20));
        assert_eq!(self.metrics.plug_fails.count(), plug_fails);
    }
    ...
    fn test_plug_request_too_big() {
        ...
        assert_eq!(self.metrics.plug_count.count(), plug_count + 1);
        assert_eq!(self.metrics.plug_bytes.count(), plug_bytes);
        assert_eq!(self.metrics.plug_fails.count(), plug_fails + 1);
    }
    ...
    fn test_unplug_request_success() {
        ...
        let unplug_count = self.metrics.unplug_count.count();
        let unplug_bytes = self.metrics.unplug_bytes.count();
        let unplug_fails = self.metrics.unplug_fails.count();
        ...
        assert_eq!(self.metrics.unplug_count.count(), unplug_count + 1);
        assert_eq!(self.metrics.unplug_bytes.count(), unplug_bytes + (2 << 20));
        assert_eq!(self.metrics.unplug_fails.count(), unplug_fails);
    }
    ...
    fn test_unplug_request_not_plugged() {
        ...
        assert_eq!(self.metrics.unplug_count.count(), unplug_count + 1);
        assert_eq!(self.metrics.unplug_bytes.count(), unplug_bytes);
        assert_eq!(self.metrics.unplug_fails.count(), unplug_fails + 1);
    }
    ...
    fn test_unplug_all_request() {
        ...
        let unplug_all_count = self.metrics.unplug_all_count.count();
        let unplug_all_fails = self.metrics.unplug_all_fails.count();
        ...
        assert_eq!(self.metrics.unplug_all_count.count(), unplug_all_count + 1);
        assert_eq!(self.metrics.unplug_all_fails.count(), unplug_all_fails);
    }
    ...
    fn test_state_request_unplugged() {
        ...
        let state_count = self.metrics.state_count.count();
        let state_fails = self.metrics.state_fails.count();
        ...
        assert_eq!(self.metrics.state_count.count(), state_count + 1);
        assert_eq!(self.metrics.state_fails.count(), state_fails);
    }
    ...
    fn test_invalid_range_unaligned() {
        ...
        assert_eq!(self.metrics.state_count.count(), state_count + 1);
        assert_eq!(self.metrics.state_fails.count(), state_fails + 1);
    }
