...
mod tests {
    ...
    fn test_muxer_killq() {
        ...
        assert_eq!(ctx.muxer.metrics.conns_added.count(), 396);
    ...
    fn test_vsock_basic_metrics() {
        ...
        let mut tx_packets_count = ctx.muxer.metrics.tx_packets_count.count();
        let mut rx_packets_count = ctx.muxer.metrics.rx_packets_count.count();

        let tx_bytes_count = ctx.muxer.metrics.tx_bytes_count.count();
        let rx_bytes_count = ctx.muxer.metrics.rx_bytes_count.count();
