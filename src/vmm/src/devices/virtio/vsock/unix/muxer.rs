...
impl VsockChannel for VsockMuxer {
    ...
    fn send_pkt(&mut self, pkt: &VsockPacketTx) -> Result<(), VsockError> {
        // when we get here the packet is rst
...
impl VsockMuxer {
    ...
    pub fn new(
        ...
        let host_sock: Box<dyn Socket> = match vsock_type {
            VsockType::Seqpacket => {
                let sock = uds::UnixSeqpacketListener::bind(&host_sock_path)
                    .and_then(|sock| sock.set_nonblocking(true).map(|_| sock))
                    .map_err(VsockUnixBackendError::UnixBind)?;
                Box::new(SeqpacketListener::new(sock))
            }
            VsockType::Stream => {
    ...
    fn handle_event(&mut self, fd: RawFd, event_set: EventSet) {
        ...
        match self.listener_map.get_mut(&fd) {
            ...
            Some(EpollListener::HostSock) => {
                ...
                self.host_sock // calling accept on this file descriptor errors?
                    .accept()
    ...
    fn handle_peer_request_pkt(&mut self, pkt: &VsockPacketTx) {
        ...
        match self.vsock_type {
            ...
            VsockType::Seqpacket => {
                UnixSeqpacketConn::connect(port_path)
                    .and_then(|stream| stream.set_nonblocking(true).map(|_| stream))
                    .map_err(VsockUnixBackendError::UnixConnect)
                    .and_then(|stream| {
                        self.add_connection(
