...
struct VfioMsixState {
    ...
    register: u8,
    cap: MsixCap,
    /// TABLE and PBA emulated areas
    emulated_areas: ArrayVec<VfioBarEmulatedArea, 2>,
    config: MsixConfig,
}
...
fn vfio_init_device(
    ...
    for area in vfio_device.lock().unwrap().msix_state.emulated_areas.iter() {
        vm.common
            .mmio_bus
...
fn vfio_deinit_device(device: &VfioDevice) {
    ...
    for area in device.msix_state.emulated_areas.iter() {
        device
            .vm
            .common
            .mmio_bus
