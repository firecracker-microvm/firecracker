...
mod tests {
    ...
    fn test_calculate_bar_areas_msix_table_past_region_end() {
        ...
        let region_infos = make_region_infos(&[(
            0,
            make_region(0x4000, vec![VfioRegionInfoCap::MsixMappable]),
        )]);

        // table at offset 0x3ff8, 1 entry (16 bytes) → table_range end = 0x4008,
        // which is past the BAR's region_size of 0x4000.
        let msix_cap = MsixCap::new(0, 1, 0x3ff8, 0, 0);

        let err = calculate_bar_areas(&bars, &region_infos, Some(&msix_cap)).unwrap_err();
        assert!(matches!(
            err,
            VfioError::MsixTableOutOfRange(0, 0x3ff8, 0x10, 0x4000)
        ));
    }
    ...
    fn test_calculate_bar_areas_msix_pba_past_region_end() {
        ...
        // PBA at offset 0x4000 (= region_size), 1 entry → pba_range size = 8 bytes,
        // end = 0x4008, past region_size 0x4000.
        let msix_cap = MsixCap::new(0, 1, 0, 0, 0x4000);

        let err = calculate_bar_areas(&bars, &region_infos, Some(&msix_cap)).unwrap_err();
        assert!(matches!(
            err,
            VfioError::MsixPbaOutOfRange(0, 0x4000, 0x8, 0x4000)
        ));
    }
