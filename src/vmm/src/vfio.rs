...
pub struct VfioMsixState {
    ...
    pub bar_hole_infos: ArrayVec<VfioBarHole, 2>,
    /// Config
        ...
        if size.is_power_of_two() {
            ...
            if is_io_bar {
                ...
                match resource_allocator.mmio64_memory.allocate(
                    ...
                    Err(_) => {
                        vfio_deallocate_bars(resource_allocator, &bars);
...
mod tests {
    ...
    fn holes_table_only() -> ArrayVec<VfioBarHole, 2> {
        let mut holes = ArrayVec::new();
        holes.push(VfioBarHole {
            gpa: BAR_GPA,
            size: 0x1000,
            usage: VfioBarHoleUsageFlags::TABLE,
        });
    ...
    fn holes_pba_only() -> ArrayVec<VfioBarHole, 2> {
        let mut holes = ArrayVec::new();
        holes.push(VfioBarHole {
            gpa: BAR_GPA,
            size: 0x1000,
    ...
    fn holes_merged() -> ArrayVec<VfioBarHole, 2> {
        let mut holes = ArrayVec::new();
        holes.push(VfioBarHole {
            gpa: BAR_GPA,
            size: 0x1000,
