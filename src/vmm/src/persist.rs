...
use crate::version_map::{
    FC_V1_0_SNAP_VERSION, FC_V1_1_SNAP_VERSION, FC_VERSION_TO_SNAP_VERSION,
    FC_VERSION_TO_SNAP_VERSION,
};
...
use crate::vstate::{self};
use crate::{
    mem_size_mib, memory_snapshot, Error as VmmError, Error as VmmError, EventManager,
    EventManager, Vmm, Vmm,
};
...
pub enum CreateSnapshotError {
    ...
    /// The virtio devices uses a features that is incompatible with older
    /// versions of Firecracker.
    ...
    #[cfg(target_arch = "x86_64")]
    /// Number of devices exceeds the maximum supported devices for the snapshot
    /// data version.
...
pub enum LoadSnapshotError {
    ...
    /// Failed to send guest memory layout and path to user fault FD used to
    /// handle guest memory page faults. This information is sent to a UDS
    /// where a custom page-fault handler process is listening.
...
/// Validate the microVM version and translate it to its corresponding snapshot
/// data format.
...
pub fn snapshot_state_sanity_check(
    ...
    // Upper bound check will be done when creating guest memory by comparing
    // against KVM max supported value kvm_context.max_memslots().
