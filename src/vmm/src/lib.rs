...
//! Virtual Machine Monitor that leverages the Linux Kernel-based Virtual
//! Machine (KVM), and other virtualization features to run a single lightweight
//! micro-virtual machine (microVM).
#![deny(missing_docs)]
...
// Since the exit code names e.g. `SIGBUS` are most appropriate yet trigger a
// test error with the clippy lint `upper_case_acronyms` we have disabled this
// lint for this enum.
/// Vmm exit-code type.
...
pub enum FcExitCode {
    ...
    GenericError = 1,
    /// Generic exit code for an error considered not possible to occur if the
...
/// Pause/Resume/Save/Restore. A high enough limit that should not be reached
/// during normal usage, used to detect a potential vcpu deadlock.
...
/// Errors associated with the VMM internal logic. These errors cannot be
/// generated by direct user input, but can result from bad configuration of the
/// host (for example if Firecracker doesn't have permissions to open the KVM
/// fd).
#[derive(Debug)]
pub enum Error {
    /// Legacy devices work with Event file descriptors and the creation can
    /// fail because of resource exhaustion.
    #[cfg(target_arch = "x86_64")]
    ...
    KernelFile(io::Error),
    /// Cannot open /dev/kvm. Either the host does not have KVM or Firecracker
    /// does not have permission to open the file descriptor.
...
/// Trait for objects that need custom initialization and teardown during the
/// Vmm lifetime.
...
impl Vmm {
    ...
    pub fn set_dirty_page_tracking(&mut self, enable: bool) -> Result<()> {
        // This function _always_ results in an ioctl update. The VMM is stateless in
        // the sense that it's unaware of the current dirty page tracking
        // setting. The VMM's consumer will need to cache the dirty tracking
        // setting internally. For example, if this function were to be exposed
        // through the VMM controller, the VMM resources should cache the flag.
        self.vm
            .set_kvm_memory_regions(&self.guest_memory, enable)
            .map_err(Error::Vm)
    }

    /// Updates the path of the host file backing the emulated block device with
    /// id `drive_id`. We update the disk image on the device and its virtio
    /// configuration.
    ...
    /// Updates configuration for the balloon device as described in
    ...
    pub fn stop(&mut self, exit_code: FcExitCode) {
        ...
        */
        ...
        // The actual thread::join() that runs to release the thread's resource is done
        // in the VcpuHandle's Drop trait.  We can trigger that to happen now by
        // clearing the list of handles. Do it here instead of Vmm::Drop to
        // avoid dependency cycles. (Vmm's Drop will also check if this list is
        // empty).
...
/// Process the content of the MPIDR_EL1 register in order to be able to pass it
/// to KVM
///
/// The kernel expects to find the four affinity levels of the MPIDR in the
/// first 32 bits of the VGIC register attribute:
