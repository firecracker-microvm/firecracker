...
impl Vmm {
    ...
    pub fn full_config(&self) -> VmmConfig {
        ...
        let mut device_passthrough = Vec::new();
        ...
        if let Some(pci_devices) = self.device_manager.pci_devices() {
            for device in pci_devices.vfio_devices.iter() {
                device_passthrough.push(device.lock().unwrap().config.clone());
            }
        }
    ...
    pub fn check_unsnapshottable_devices(&self) -> Result<(), MicrovmStateError> {
        ...
        if let Some(pci_devices) = self.device_manager.pci_devices() {
            for device in pci_devices.vfio_devices.iter() {
                tuples.push(("passthrough device", device.lock().unwrap().config.id.clone()));
            }
        }
        if tuples.is_empty() {
            Ok(())
