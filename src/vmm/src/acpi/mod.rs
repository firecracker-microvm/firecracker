...
impl<'a> AcpiTableWriter<'a> {
    ...
    fn write_acpi_table<S>(&self, table: &mut S) -> Result<u64, AcpiError>
    where
        S: Sdt,
    {
    ...
    fn build_fadt(&self, dsdt_addr: u64) -> Result<u64, AcpiError> {
    ...
    fn build_madt(&self, nr_vcpus: u8) -> Result<u64, AcpiError> {
    ...
    fn build_xsdt(&self, fadt_addr: u64, madt_addr: u64) -> Result<u64, AcpiError> {
        debug!("acpi: Building XSDT table");
    ...
    fn build_rsdp(&self, xsdt_addr: u64) -> Result<(), AcpiError> {
        debug!("acpi: Building RSDP");
        let mut rsdp = Rsdp::new(OEM_ID, xsdt_addr);
        rsdp.write_to_guest(self.mem, rsdp_addr())
            .inspect_err(|err| debug!("acpi: Could not write RSDP in guest memory: {err}"))?;
...
pub mod tests {
    ...
    use crate::arch::x86_64::layout::{ACPI_MEM_SIZE, ACPI_MEM_START};
    ...
    // Currently we are allocating three 4K pages for ACPI tables. We are allocating using the
    // FirstMatch policy, with an 1 byte alignment. This test checks that we are able to allocate
    // up to this size, and get back the expected addresses.
    #[test]
    fn test_write_acpi_table_memory_allocation() {
        ...
        assert_eq!(addr, ACPI_MEM_START);

        // Let's try to write two 4K pages plus one byte
        let mut sdt = MockSdt(vec![0; 8193]);
        ...
        let addr = writer.write_acpi_table(&mut sdt).unwrap();
        assert_eq!(addr, ACPI_MEM_START + 4096);
        let mut sdt = MockSdt(vec![0; 2]);
        let addr = writer.write_acpi_table(&mut sdt).unwrap();
        assert_eq!(addr, ACPI_MEM_START + 4101);
        let mut sdt = MockSdt(vec![0; 4]);
        let addr = writer.write_acpi_table(&mut sdt).unwrap();
        assert_eq!(addr, ACPI_MEM_START + 4103);
        let mut sdt = MockSdt(vec![0; 8]);
        let addr = writer.write_acpi_table(&mut sdt).unwrap();
        assert_eq!(addr, ACPI_MEM_START + 4107);
        let mut sdt = MockSdt(vec![0; 16]);
        let addr = writer.write_acpi_table(&mut sdt).unwrap();
        assert_eq!(addr, ACPI_MEM_START + 4115);
    }
    ...
    fn test_write_acpi_table_small_memory() {
        ...
        vmm.guest_memory = arch_mem((ACPI_MEM_START + ACPI_MEM_SIZE - 4096).try_into().unwrap());
        let writer = AcpiTableWriter {
            mem: vmm.guest_memory(),
            resource_allocator: vmm.resource_allocator.clone(),
        };

        let mut sdt = MockSdt(vec![0; 4096 * 3]);
        let err = writer.write_acpi_table(&mut sdt).unwrap_err();
        assert!(
            matches!(
                err,
