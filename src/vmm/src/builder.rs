...
use crate::vstate::vm::{
    GUEST_MEMFD_FLAG_MMAP, GUEST_MEMFD_FLAG_NO_DIRECT_MAP, KvmVm, Vm, VmError,
};
...
pub fn build_microvm_for_boot(
    ...
    let mut vcpus = vm.create_vcpus(vm_resources.machine_config.vcpu_count, secret_free)?;
    ...
    let virtio_mem_addr = if let Some(memory_hotplug) = &vm_resources.memory_hotplug {
        ...
        let guest_memfd_offset = guest_memfd
            .is_some()
            .then(|| vm_resources.dram_memory_size() as u64);
...
pub fn build_microvm_from_snapshot(
    ...
    // Set up KVM VM and register memory regions.
    // Build custom CPU config if a custom template is provided.
    let mut vm = KvmVm::new(kvm, secret_free).map_err(StartMicrovmError::KvmVm)?;
...
pub(crate) mod tests {
    ...
    pub(crate) fn default_vmm() -> Vmm {
        ...
        let _ = vm.create_vcpus(1, false).unwrap();
