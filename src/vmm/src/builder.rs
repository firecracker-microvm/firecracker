...
pub mod tests {
    ...
    use seccomp::SECCOMP_LEVEL_ADVANCED;
    use std::fs;
    ...
    fn default_vmm() -> Vmm {
        ...
        Vmm {
            ...
            #[cfg(target_arch = "x86_64")]
            pio_device_manager,
            seccomp_level: SECCOMP_LEVEL_ADVANCED,
        }
    }
    ...
    fn test_attach_vsock_device() {
        ...
        let tmp_sock_file = TempSockFile::new(TempFile::new("/tmp/").unwrap());
    ...
    fn test_error_messages() {
        ...
        let err = GuestMemory(GuestMemoryError::NoMemoryRegions);
        assert_eq!(
            format!("{}", err),
            format!(
