...
pub struct PciDevices {
    ...
    pub virtio_devices: HashMap<u32, Arc<Mutex<VirtioPciDevice>>>,
}
...
impl PciDevices {
    ...
    >(
        ...
        self.virtio_devices
            .insert(pci_device_bdf.into(), virtio_device.clone());
    ...
    fn restore_pci_device<T: 'static + VirtioDevice + MutEventSubscriber + Debug>(
        ...
        // We should only be reaching this point if PCI is enabled
        let pci_segment = self.pci_segment.as_ref().unwrap();
        let msi_vector_group = Arc::new(MsiVectorGroup::from_state(
            vm.clone(),
        ...
        pci_segment
            ...
            .lock()
            .expect("Poisoned lock")
            .add_device(pci_device_bdf, virtio_device.clone())?;

        self.virtio_devices
            .insert(pci_device_bdf, virtio_device.clone());
...
impl<'a> Persist<'a> for PciDevices {
    ...
    fn save(&self) -> Self::State {
        ...
        for (dev_id, pci_dev) in &self.virtio_devices {
