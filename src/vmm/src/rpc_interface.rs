...
use crate::vmm_config::balloon::{
    BalloonDeviceSpec, BalloonSpecError, BalloonStats, BalloonUpdateSpec, BalloonUpdateStatsSpec,
};
use crate::vmm_config::boot_source::{BootSourceSpec, BootSourceSpecError};
...
use crate::vmm_config::memory_hotplug::{
    MemoryHotplugSizeUpdate, MemoryHotplugSpec, MemoryHotplugSpecError,
};
use crate::vmm_config::metrics::{MetricsSpec, MetricsSpecError};
...
use crate::vmm_config::pmem::{PmemSpec, PmemSpecError};
use crate::vmm_config::serial::SerialSpec;
use crate::vmm_config::snapshot::{CreateSnapshotParams, LoadSnapshotParams, SnapshotType};
use crate::vmm_config::vsock::{VsockDeviceSpec, VsockSpecError};
...
pub enum VmmData {
    ...
    Empty,
    /// The complete microVM specification in JSON format.
    FullVmSpec(VmmSpec),
...
impl<'a> PrebootApiController<'a> {
    ...
    pub fn handle_preboot_request(
        ...
        match request {
            ...
            ConfigureBootSource(spec) => self.set_boot_source(spec),
            ConfigureLogger(spec) => crate::logger::LOGGER
                .update(spec)
                .map(|()| VmmData::Empty)
                .map_err(VmmActionError::Logger),
            ConfigureMetrics(spec) => vmm_config::metrics::init_metrics(spec)
                .map(|()| VmmData::Empty)
                .map_err(VmmActionError::Metrics),
            ConfigureSerial(spec) => {
                self.vm_resources.serial_out_path = spec.serial_out_path;
                Ok(VmmData::Empty)
            }
            ...
            GetVmMachineSpec => Ok(VmmData::MachineSpec(self.vm_resources.machine_spec.clone())),
            ...
            InsertBlockDevice(spec) => self.insert_block_device(spec),
            InsertPmemDevice(spec) => self.insert_pmem_device(spec),
            InsertNetworkDevice(spec) => self.insert_net_device(spec),
            LoadSnapshot(load_params) => self
                .load_snapshot(&load_params)
            ...
            SetBalloonDevice(spec) => self.set_balloon_device(spec),
            SetVsockDevice(spec) => self.set_vsock_device(spec),
            SetMmdsSpec(spec) => self.set_mmds_config(spec),
            StartMicroVm => self.start_microvm(),
            UpdateMachineSpec(spec_update) => self.update_machine_spec(spec_update),
            SetEntropyDevice(spec) => self.set_entropy_device(spec),
            SetMemoryHotplugDevice(spec) => self.set_memory_hotplug_device(spec),
    ...
    fn insert_block_device(&mut self, spec: BlockDeviceSpec) -> Result<VmmData, VmmActionError> {
        self.boot_path = true;
        self.vm_resources
            .set_block_device(spec)
            .map(|()| VmmData::Empty)
            .map_err(VmmActionError::DriveSpec)
    }

    fn insert_net_device(&mut self, spec: NetworkInterfaceSpec) -> Result<VmmData, VmmActionError> {
        self.boot_path = true;
        self.vm_resources
            .build_net_device(spec)
            .map(|()| VmmData::Empty)
            .map_err(VmmActionError::NetworkSpec)
    }

    fn insert_pmem_device(&mut self, spec: PmemSpec) -> Result<VmmData, VmmActionError> {
        self.boot_path = true;
        self.vm_resources
            .build_pmem_device(spec)
            .map(|()| VmmData::Empty)
            .map_err(VmmActionError::PmemDevice)
    }

    fn set_balloon_device(&mut self, spec: BalloonDeviceSpec) -> Result<VmmData, VmmActionError> {
        self.boot_path = true;
        self.vm_resources
            .set_balloon_device(spec)
            .map(|()| VmmData::Empty)
            .map_err(VmmActionError::BalloonSpec)
    }

    fn set_boot_source(&mut self, spec: BootSourceSpec) -> Result<VmmData, VmmActionError> {
        self.boot_path = true;
        self.vm_resources
            .build_boot_source(spec)
            .map(|()| VmmData::Empty)
            .map_err(VmmActionError::BootSource)
    }

    fn set_mmds_config(&mut self, spec: MmdsSpec) -> Result<VmmData, VmmActionError> {
        self.boot_path = true;
        self.vm_resources
            .set_mmds_config(spec, &self.instance_info.id)
            .map(|()| VmmData::Empty)
            .map_err(VmmActionError::MmdsSpec)
    }

    fn update_machine_spec(
        &mut self,
        update: MachineSpecUpdate,
    ) -> Result<VmmData, VmmActionError> {
        self.boot_path = true;
        self.vm_resources
            .update_machine_spec(&update)
            .map(|()| VmmData::Empty)
    ...
    fn set_vsock_device(&mut self, spec: VsockDeviceSpec) -> Result<VmmData, VmmActionError> {
        self.boot_path = true;
        self.vm_resources
            .set_vsock_device(spec)
            .map(|()| VmmData::Empty)
            .map_err(VmmActionError::VsockSpec)
    }

    fn set_entropy_device(&mut self, spec: EntropyDeviceSpec) -> Result<VmmData, VmmActionError> {
        self.boot_path = true;
        self.vm_resources.build_entropy_device(spec)?;
        Ok(VmmData::Empty)
    }
    ...
    fn set_memory_hotplug_device(
        ...
        self.boot_path = true;
        self.vm_resources.set_memory_hotplug_config(spec)?;
        Ok(VmmData::Empty)
    }
...
impl RuntimeApiController {
    ...
    pub fn handle_request(&mut self, request: VmmAction) -> Result<VmmData, VmmActionError> {
        ...
        match request {
            ...
            StopFreePageHinting => self
                ...
                .map(|_| VmmData::Empty)
                .map_err(VmmActionError::BalloonUpdate),
            UpdateBlockDevice(new_spec) => self.update_block_device(new_spec),
    ...
    fn update_block_device(
        &mut self,
        new_spec: BlockDeviceUpdateSpec,
    ) -> Result<VmmData, VmmActionError> {
        let mut vmm = self.vmm.lock().expect("Poisoned lock");

        // vhost-user-block updates
        if new_spec.path_on_host.is_none() && new_spec.rate_limiter.is_none() {
            vmm.update_vhost_user_block_config(&new_spec.drive_id)
                .map_err(DriveError::DeviceUpdate)?;
        }

        // virtio-block updates
        if let Some(new_path) = new_spec.path_on_host {
            vmm.update_block_device_path(&new_spec.drive_id, new_path)
                .map_err(DriveError::DeviceUpdate)?;
        }
        if new_spec.rate_limiter.is_some() {
            vmm.update_block_rate_limiter(
                &new_spec.drive_id,
                RateLimiterUpdate::from(new_spec.rate_limiter).bandwidth,
                RateLimiterUpdate::from(new_spec.rate_limiter).ops,
            )
            .map_err(DriveError::DeviceUpdate)?;
        }
        Ok(VmmData::Empty)
    ...
    /// Updates configuration for an emulated net device as described in `new_spec`.
    fn update_net_rate_limiters(
        &mut self,
        new_spec: NetworkInterfaceUpdateSpec,
    ) -> Result<VmmData, VmmActionError> {
        self.vmm
            .lock()
            .expect("Poisoned lock")
            .update_net_rate_limiters(
                &new_spec.iface_id,
                RateLimiterUpdate::from(new_spec.rx_rate_limiter).bandwidth,
                RateLimiterUpdate::from(new_spec.rx_rate_limiter).ops,
                RateLimiterUpdate::from(new_spec.tx_rate_limiter).bandwidth,
                RateLimiterUpdate::from(new_spec.tx_rate_limiter).ops,
            )
            .map(|()| VmmData::Empty)
