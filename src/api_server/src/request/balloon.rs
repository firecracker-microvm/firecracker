...
pub(crate) fn parse_put_balloon(body: serde_json::Value) -> Result<ParsedRequest, Error> {
    Ok(ParsedRequest::new_sync(VmmAction::SetBalloonDevice(
        serde_json::from_value::<BalloonDeviceConfig>(body)?,
    )))
}
    ...
    match path_second_token {
        Some(config_path) => match config_path {
            "statistics" => Ok(ParsedRequest::new_sync(VmmAction::UpdateBalloonStatistics(
                serde_json::from_value::<BalloonUpdateStatsConfig>(body)?,
            ))),
            _ => Err(Error::Generic(
                StatusCode::BAD_REQUEST,
        ...
        None => Ok(ParsedRequest::new_sync(VmmAction::UpdateBalloon(
            serde_json::from_value::<BalloonUpdateConfig>(body)?,
        ))),
    }
}
...
mod tests {
    ...
    fn test_parse_patch_balloon_request() {
        assert!(parse_patch_balloon(serde_json::Value::Null, None).is_err());
        ...
        assert!(parse_patch_balloon(body, None).is_err());
        ...
        let res = parse_patch_balloon(body, None);
        assert!(res.is_err());
        ...
        let res = parse_patch_balloon(body, Some("statistics"));
        assert!(res.is_err());
        ...
        assert!(parse_patch_balloon(body, Some("config")).is_err());

        let body = json!({
          "amount_mib": 1
        });
        #[allow(clippy::match_wild_err_arm)]
        match vmm_action_from_request(parse_patch_balloon(body, None).unwrap()) {
        ...
        #[allow(clippy::match_wild_err_arm)]
        match vmm_action_from_request(parse_patch_balloon(body, Some("statistics")).unwrap()) {
    ...
    fn test_parse_put_balloon_request() {
        assert!(parse_put_balloon(serde_json::Value::Null).is_err());
        ...
        assert!(parse_put_balloon(body).is_err());
        ...
        assert!(parse_put_balloon(body).is_ok());
    }
}
