...
//! # Rate Limiter
//!
//! Provides a rate limiter written in Rust useful for IO operations that need
//! to be throttled.
//!
//! ## Behavior
//!
...
//! on the `TokenType` used. If any of the buckets runs out of budget, the
//! limiter goes in the 'blocked' state. At this point an internal timer is set
//! up which will later 'wake up' the user in order to retry sending data. The
//! 'wake up' notification will be dispatched as an event on the FD provided by
//! the `AsRawFD` trait implementation.
//!
//! The contract is that the user shall also call the `event_handler()` method
//! on receipt of such an event.
//!
//! The token buckets are replenished when a called `consume()` doesn't find
//! enough tokens in the bucket. The amount of tokens replenished is
//! automatically calculated to respect the `complete_refill_time` configuration
//! parameter provided by the user. The token buckets will never replenish above
//! their respective `size`.
//!
//! Each token bucket can start off with a `one_time_burst` initial extra
//! capacity on top of their `size`. This initial extra credit does not
//! replenish and can be used for an initial burst of data.
//!
...
//! ## Limitations
//!
//! This rate limiter implementation relies on the *Linux kernel's timerfd* so
//! its usage is limited to Linux systems.
//!
//! Another particularity of this implementation is that it is not self-driving.
//! It is meant to be used in an external event loop and thus implements the
//! `AsRawFd` trait and provides an *event-handler* as part of its API. This
//! *event-handler* needs to be called by the user on every event on the rate
//! limiter's `AsRawFd` FD.
...
pub enum BucketReduction {
    ...
    Success,
    /// A number of tokens `inner` times larger than the bucket size have been
    /// consumed.
    OverConsumption(f64),
}
...
impl TokenBucket {
    ...
    ///
    /// TokenBucket created is of `size` total capacity and takes
    /// `complete_refill_time_ms` milliseconds to go from zero tokens to
    /// total capacity. The `one_time_burst` is initial extra credit on top
    /// of total capacity, that does not replenish and which can be used for
    /// an initial burst of data.
    ///
    /// If the `size` or the `complete refill time` are zero, then `None` is
    /// returned.
    pub fn new(size: u64, one_time_burst: u64, complete_refill_time_ms: u64) -> Option<Self> {
        ...
        // refill_token_count = (delta_time * size) / (complete_refill_time_ms *
        // 1_000_000) In order to avoid overflows, simplify the fractions by
    ...
    // Replenishes token bucket based on elapsed time. Should only be called
    // internally by `Self`.
    fn auto_replenish(&mut self) {
        ...
        // `processed_capacity` and `processed_refill_time` are the result of
        // simplifying above fraction formula with their greatest-common-factor.
    ...
    /// Attempts to consume `tokens` from the bucket and returns whether the
    /// action succeeded.
    pub fn reduce(&mut self, mut tokens: u64) -> BucketReduction {
        ...
        if self.one_time_burst > 0 {
            ...
            if self.one_time_burst >= tokens {
                ...
                self.last_update = Instant::now();
                // No need to continue to the refill process, we still have burst budget to
                // consume from.
    ...
    pub fn force_replenish(&mut self, tokens: u64) {
        ...
        // Of course there is a very small chance  that the last reduce() also used up
        // burst budget which should now be replenished, but for performance and
        // code-complexity reasons we're just gonna let that slide since it's
    ...
    /// Returns the time in milliseconds required to to completely fill the
    /// bucket.
...
impl RateLimiter {
    ...
    /// # Arguments
    ///
    /// * `bytes_total_capacity` - the total capacity of the `TokenType::Bytes`
    ///   token bucket.
    /// * `bytes_one_time_burst` - initial extra credit on top of
    ///   `bytes_total_capacity`,
    /// that does not replenish and which can be used for an initial burst of
    /// data.
    /// * `bytes_complete_refill_time_ms` - number of milliseconds for the
    ///   `TokenType::Bytes`
    /// token bucket to go from zero Bytes to `bytes_total_capacity` Bytes.
    /// * `ops_total_capacity` - the total capacity of the `TokenType::Ops`
    ///   token bucket.
    /// * `ops_one_time_burst` - initial extra credit on top of
    ///   `ops_total_capacity`,
    /// that does not replenish and which can be used for an initial burst of
    /// data.
    /// * `ops_complete_refill_time_ms` - number of milliseconds for the
    ///   `TokenType::Ops` token
    ...
    pub fn new(
        ...
        // We'll need a timer_fd, even if our current config effectively disables rate
        // limiting, because `Self::update_buckets()` might re-enable it later,
        // and we might be seccomp-blocked from creating the timer_fd at that
        // time.
    ...
    ///
    /// If rate limiting is disabled on provided `token_type`, this function
    /// will always succeed.
    pub fn consume(&mut self, tokens: u64, token_type: TokenType) -> bool {
        // If the timer is active, we can't consume tokens from any bucket and the
        // function fails.
    ...
    ///
    /// The limiter 'blocks' when a `consume()` operation fails because there
    /// was not enough budget for it.
    /// An event will be generated on the exported FD when the limiter
    /// 'unblocks'.
    ...
    /// Updates the parameters of the token buckets associated with this
    /// RateLimiter.
    // TODO: Please note that, right now, the buckets become full after being
    // updated.
...
pub(crate) mod tests {
    ...
    impl TokenBucket {
        // Resets the token bucket: budget set to max capacity and last-updated set to
        // now.
        fn reset(&mut self) {
        ...
        // After a restore, we cannot be certain that the last_update field has the same
        // value.
    ...
    fn test_update_buckets() {
        ...
        // We have manually adjust the last_update field, because it changes when
        // update_buckets() constructs new buckets (and thus gets a different
        // value for last_update). We do this so it makes sense to test the
