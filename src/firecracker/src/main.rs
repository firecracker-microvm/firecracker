...
enum MainError {
    ...
    InvalidLogLevel(<log::LevelFilter as std::str::FromStr>::Err),
    #[error("Could not initialize logger: {0}")]
    LoggerInitialization(vmm::vmm_config::logger::LoggerConfigApplyError),
...
fn main_exec() -> Result<(), MainError> {
    ...
    let level = arguments
        .single_value("level")
        .map(|s| vmm::vmm_config::logger::LevelFilter::from_str(s))
        .transpose()
        .map_err(MainError::InvalidLogLevel)?;
    let show_level = arguments.flag_present("show-level").then(|| true);
    let show_log_origin = arguments.flag_present("show-level").then(|| true);
    LoggerConfig {
        log_path,
        level,
        show_level,
