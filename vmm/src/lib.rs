...
impl EpollContext {
    ...
    /// `dispatch_table`.
    fn add_epollin_event<T: AsRawFd>(&mut self, fd: &T, token: EpollDispatch) -> Result<()> {
    ...
    fn add_epollout_event<T: AsRawFd>(&mut self, fd: &T, token: EpollDispatch) -> Result<()> {
        // The index in the dispatch where the new token will be added.
        let dispatch_index = self.dispatch_table.len() as u64;
    ...
    fn del_epollout_event<T: AsRawFd>(&mut self, fd: &T, _token: EpollDispatch) -> Result<()> {
        // Delete an epoll event on `fd`.
        epoll::ctl(
            self.epoll_raw_fd,
            epoll::ControlOptions::EPOLL_CTL_DEL,
            fd.as_raw_fd(),
            epoll::Event::new(epoll::Events::EPOLLOUT, 0),
        )
        .map_err(Error::EpollFd)?;
...
pub struct Vmm {
    ...
    // API resources.
    control_write_fd: EventFd,
    api_request_receiver: Receiver<Box<(VmmAction, Token)>>,
    api_response_sender: Sender<Box<(VmmRequestOutcome, Token)>>,
    transit_queue: VecDeque<(VmmRequestOutcome, Token)>,
...
impl Vmm {
    ...
    pub fn new(
        ...
        epoll_context
            .add_epollin_event(control_read_fd, EpollDispatch::VmmActionRequest)
            .expect("Cannot add API control_read_fd to epoll.");
    ...
    fn run_event_loop(&mut self) -> Result<EventLoopExitReason> {
        ...
        loop {
            ...
            match self.epoll_context.dispatch_table[event.data as usize] {
                ...
                Some(EpollDispatch::VmmActionResponse) => {
                    println!("VMM sending an action response to api");
                    let (vmm_request_outcome, token) = self.transit_queue.pop_front().unwrap();
                    self.control_write_fd.write(1).unwrap();
                    self.api_response_sender
                        .send(Box::new((vmm_request_outcome, token)))
                        .unwrap();

                    if self.transit_queue.is_empty() {
                        self.epoll_context.del_epollout_event(
                            &self.control_write_fd,
                            EpollDispatch::VmmActionResponse,
                        )?;
                    }
                }
    ...
    fn send_response(&mut self, outcome: VmmRequestOutcome, token: Token) -> Result<()> {
        if self.transit_queue.is_empty() {
            self.epoll_context
                .add_epollout_event(&self.control_write_fd, EpollDispatch::VmmActionResponse)?;
        }

        self.transit_queue.push_back((outcome, token));
        Ok(())
    }
    ...
    fn run_vmm_action(&mut self) -> Result<()> {
        ...
        let (request, token) = match self.api_request_receiver.try_recv() {
            Ok(t) => *t,
        ...
        match request {
            ConfigureBootSource(boot_source_body) => {
                let outcome = self.configure_boot_source(
                    boot_source_body.kernel_image_path,
                    boot_source_body.boot_args,
                );
                self.send_response(outcome, token);
            }
            ConfigureLogger(logger_description) => {
                let outcome = self.init_logger(logger_description);
                self.send_response(outcome, token);
            }
            FlushMetrics => {
                let outcome = self.flush_metrics();
                self.send_response(outcome, token);
            }
            GetVmConfiguration => {
                let outcome = Ok(VmmData::MachineConfiguration(self.vm_config.clone()));
                self.send_response(outcome, token);
            }
            InsertBlockDevice(block_device_config) => {
                let outcome = self.insert_block_device(block_device_config);
                self.send_response(outcome, token);
            }
            InsertNetworkDevice(netif_body) => {
                let outcome = self.insert_net_device(netif_body);
                self.send_response(outcome, token);
            }
            SetVsockDevice(vsock_cfg) => {
                let outcome = self.set_vsock_device(vsock_cfg);
                self.send_response(outcome, token);
            }
            RescanBlockDevice(drive_id) => {
                let outcome = self.rescan_block_device(&drive_id);
                self.send_response(outcome, token);
            }
            StartMicroVm => {
                let outcome = self.start_microvm();
                self.send_response(outcome, token);
            }
            SendCtrlAltDel => {
                let outcome = self.send_ctrl_alt_del();
                self.send_response(outcome, token);
            }
            SetVmConfiguration(machine_config_body) => {
                let outcome = self.set_vm_configuration(machine_config_body);
                self.send_response(outcome, token);
            }
            UpdateBlockDevicePath(drive_id, path_on_host) => {
                let outcome = self.set_block_device_path(drive_id, path_on_host);
                self.send_response(outcome, token);
            }
            UpdateNetworkInterface(netif_update) => {
                let outcome = self.update_net_device(netif_update);
                self.send_response(outcome, token);
            }
        };
        Ok(())
...
impl PartialEq for VmmAction {
    fn eq(&self, other: &VmmAction) -> bool {
        ...
        match (self, other) {
            ...
            (
                &SetVmConfiguration(ref vm_config),
                &SetVmConfiguration(ref other_vm_config),
...
/// * `api_request_receiver` - The receiver end point of the communication channel.
...
mod tests {
    ...
    fn create_vmm_object(state: InstanceState) -> Vmm {
        ...
        let (_to_vmm, api_request_receiver) = channel();
        let (api_response_sender, _to_api) = channel();
        Vmm::new(
            shared_info,
    ...
    #[test]
    fn test_init_logger_api_request_receiver() {
