...
pub struct Vmm {
    ...
    // API resources.
    api_request_receiver: Receiver<Box<VmmAction>>,
    api_response_sender: Sender<Box<VmmRequestOutcome>>,
...
impl Vmm {
    ...
    fn send_response(&mut self, outcome: VmmRequestOutcome) -> Result<()> {
        self.api_response_sender
            .send(Box::new(outcome))
            .map_err(|_| Error::ApiChannel)?;
        Ok(())
    }
    ...
    fn run_vmm_action(&mut self) -> Result<()> {
        ...
        let request = match self.api_request_receiver.try_recv() {
            Ok(t) => *t,
