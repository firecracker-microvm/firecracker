...
impl Vmm {
    ...
    fn load_kernel(&mut self) -> std::result::Result<GuestAddress, StartMicrovmError> {
        ...
        // This is x86_64 specific since on aarc64 the commandline will be specified through the FDT.
        #[cfg(target_arch = "x86_64")]
...
mod tests {
    ...
    fn good_kernel_file() -> String {
        ...
        return format!(
            ...
            "/kernel/src/loader/test_elf.bin".to_string()
        );
        #[cfg(target_arch = "aarch64")]
        return format!(
            "{}{}",
            parent.to_str().unwrap(),
            "kernel/src/loader/test_pe.bin".to_string()
        );
    }
    ...
    impl Vmm {
        ...
        fn default_kernel_config(&mut self, cust_kernel_path: Option<String>) {
            let kernel_path;
            let kernel_file_temp =
                NamedTempFile::new().expect("Failed to create temporary kernel file.");
            if cust_kernel_path.is_none() {
                let kernel_file_temp_path = kernel_file_temp.path().to_path_buf();
                kernel_path = String::from(kernel_file_temp_path.to_str().unwrap());
            } else {
