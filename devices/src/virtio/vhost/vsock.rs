use super::*;
use super::handle::*;
use super::super::{ActivateError, ActivateResult};
...
impl Vsock {
    ...
    pub fn new(cid: u64, mem: &GuestMemory, epoll_config: VhostEpollConfig) -> Result<Vsock> {
        ...
        let avail_features = 1 << VIRTIO_F_NOTIFY_ON_EMPTY | 1 << VIRTIO_RING_F_INDIRECT_DESC
            | 1 << VIRTIO_RING_F_EVENT_IDX | 1 << VHOST_F_LOG_ALL
            | 1 << VIRTIO_F_ANY_LAYOUT | 1 << VIRTIO_F_VERSION_1;
    ...
    pub fn new_for_testing(cid: u64, features: u64, epoll_config: VhostEpollConfig) -> Vsock {
        Vsock {
        ...
        if let Some(vsock_fd) = self.vsock_fd.take() {
            if let Some(interrupt) = self.interrupt.take() {
                if let Some(kill_evt) = self.worker_kill_evt.take() {
                    ...
                    let handler = VhostEpollHandler::new(
                        vsock_fd,
                        interrupt_status,
                        interrupt_evt,
                        interrupt,
                    ...
                    epoll::ctl(
                        ...
                        epoll::EPOLL_CTL_ADD,
                        kill_raw_fd,
                        epoll::Event::new(epoll::EPOLLIN, self.epoll_config.get_kill_token()),
                    ).map_err(ActivateError::EpollCtl)?;
