...
# The crosvm block backend serves this many queues. With the backend's own
# feature bits reaching the guest, the guest takes its queue count from the
# backend's config space, so the frontend has to be configured to agree.
BACKEND_NUM_QUEUES = 16
...
def uvm_vhost_user_generic_booted_ro(uvm_vhost_user_generic_plain, rootfs):
    ...
    boot_args = (
        "reboot=k panic=1 nomodule swiotlb=noforce console=ttyS0 root=/dev/vda ro"
    )
...
def uvm_vhost_user_generic_booted_rw(uvm_vhost_user_generic_plain, rootfs):
    ...
    vm.spawn()
    boot_args = (
        "reboot=k panic=1 nomodule swiotlb=noforce console=ttyS0 root=/dev/vda rw"
    )
    if not vm.pci_enabled:
        boot_args += " pci=off"
    vm.basic_config(add_root_device=False, boot_args=boot_args)
...
def test_vhost_user_generic_block(uvm_vhost_user_generic_booted_ro):
    ...
    The guest mounting it read-only is the end-to-end check that the
    backend's own feature bits reach the guest: the frontend knows nothing
    of VIRTIO_BLK_F_RO, it just passes on what the backend offered.
    """
...
def test_vhost_user_generic_block_rw(uvm_vhost_user_generic_booted_rw):
    ...
    # Checked here rather than in the read-only test, which is expected to
    # fail, so that the metrics name is actually asserted on.
...
def test_vhost_user_generic_block_multiqueue(uvm_vhost_user_generic_plain, rootfs):
    """
    Configuring more queues than the guest driver uses must still boot.

    The guest driver sizes itself from the backend's config space, so
    configuring more queues than the backend serves leaves a surplus the
    guest never readies. Only the ready ones may be set up: initializing a
    surplus queue returns NotReady and aborts activation, which hangs the
    guest on root mount.
    """
    vm = uvm_vhost_user_generic_plain
    ...
    if not vm.pci_enabled:
        boot_args += " pci=off"
    vm.basic_config(add_root_device=False, boot_args=boot_args)
    # A read-write rootfs unique to this microVM, with more queues configured
    # than the backend serves.
    rootfs_rw = Path(vm.chroot()) / "rootfs"
    shutil.copy(rootfs, rootfs_rw)
    vm.add_vhost_user_generic_device(
        "rootfs",
        rootfs_rw,
        device_type=VIRTIO_BLK_TYPE,
        num_queues=BACKEND_NUM_QUEUES + 4,
        is_read_only=False,
    )
    vm.add_net_iface()
    vm.start()

    # Reaching the guest means root mounted over the multi-queue generic device
    # and the surplus unready queues were skipped rather than aborting activation.
    assert_dict = {"1-6": "/dev/vda"}
    _check_drives(vm, assert_dict, assert_dict.keys())
