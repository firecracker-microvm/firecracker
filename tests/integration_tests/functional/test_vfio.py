...
"""Integration tests for a VFIO passthrough"""
...
# Skip tests if no passthrough device was passed through env variables. Also use
# `xdist_group` to make sure all tests here run sequentially since we only have
# 1 device to work with
pytestmark = [
    pytest.mark.skipif(
        VFIO_SBDF is None,
        reason="No passthrough device configured (set FC_VFIO_PCI_SBDF)",
    ),
...
def uvm_with_vfio(microvm_factory, guest_kernel, rootfs):
    """Boot a microVM with the NVMe device attached."""
    ...
    vm.add_net_iface()
    vm.api.device_passthrough.put(id="nvme0", sbdf=VFIO_SBDF)
    vm.start()
    return vm
