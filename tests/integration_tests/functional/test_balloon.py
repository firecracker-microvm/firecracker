...
def create_vm(context, deflate=True, stats_interval=0):
    """Create a microvm with a balloon device."""
    ...
    logger.info(
        "Testing {} with microvm: \"{}\", kernel {}, disk {} "
        .format(
            snapshot_type,
    ...
    _, host_ip, guest_ip = microvm.ssh_network_config(
        network_config,
        '1',
        tapname="tap0"
    )
    ...
    response = microvm.balloon.put(
        ...
        stats_polling_interval_s=stats_interval
    )
...
@pytest.mark.skipif(
    ...
    reason="The balloon device is less effective on arm64 architecture"
)
...
def _test_balloon_snapshot(context):
    ...
    # Dirty 40MB of pages.
    make_guest_dirty_memory(ssh_connection, amount=(40 * MB_TO_PAGES))
    time.sleep(1)
    ...
    print("second pid: {}".format(firecracker_pid))

    # Check memory usage.
    third_reading = get_rss_mem_by_pid(firecracker_pid)
    print("third reading: {}".format(third_reading))
    ...
    # Check memory usage.
    fourth_reading = get_rss_mem_by_pid(firecracker_pid)
    print("fourth reading: {}".format(fourth_reading))

    # Inflate the balloon with another 15MB of pages.
    response = microvm.balloon.patch(
        num_pages=(25 * MB_TO_PAGES),
        stats_polling_interval_s=1
    )
    assert microvm.api_session.is_status_no_content(response.status_code)
    time.sleep(5)
...
def _test_snapshot_compatibility(context):
    ...
    # Create a snapshot with a balloon on version 0.23.0.
    ...
    try:
        ...
        logger.info(
            "Creating a snapshot with a balloon on target "
            "version 0.23.0 has failed as expected."
        )
