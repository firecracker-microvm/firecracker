...
def build_cpuid_dict(raw_cpuid_output):
    ...
    for line in raw_cpuid_output:
        ...
        if len(res):
            leaf, subleaf, eax, ebx, ecx, edx = res[0]
            cpuid_dict[(int(leaf, 16), int(subleaf, 16), "eax")] = int(eax, 16)
            cpuid_dict[(int(leaf, 16), int(subleaf, 16), "ebx")] = int(ebx, 16)
            cpuid_dict[(int(leaf, 16), int(subleaf, 16), "ecx")] = int(ecx, 16)
            cpuid_dict[(int(leaf, 16), int(subleaf, 16), "edx")] = int(edx, 16)
    return cpuid_dict


def get_guest_cpuid(vm):
    """
    Returns the guest CPUID in the form of a dictionary where the key is a tuple:
     - leaf (integer)
     - subleaf (integer)
