...
impl<'a, T: NetworkBytes> EthernetFrame<'a, T> {
    /// Interprets `bytes` as an Ethernet frame without any validity checks. May cause accessor
...
impl<'a, T: NetworkBytesMut> EthernetFrame<'a, T> {
    ...
    /// Atempts to write an incomplete Ethernet frame (whose length is currently unknown) using the
    /// specified header fields to `buf`.
    #[inline]
...
impl<'a, T: NetworkBytes> Incomplete<EthernetFrame<'a, T>> {
    /// Completes the inner frame by shrinking it to its actual length. May panic if the specified
    /// length is invalid.
