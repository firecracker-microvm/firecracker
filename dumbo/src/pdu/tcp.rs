...
bitflags! {
    /// Represents the TCP header flags, with the exception of `NS`. These values are only valid in
    /// conjunction with the flags_after_ns() function, which returns the byte containing every
    /// other flag besides `NS`.
    #[allow(missing_docs)]
    pub struct Flags: u8 {
...
impl<'a, T: NetworkBytes> TcpSegment<'a, T> {
    ...
    /// Returns the value of the `urgent pointer` field (only valid if the `URG` flag is set).
    #[inline]
    pub fn urgent_pointer(&self) -> u16 {
        self.bytes.ntohs_unchecked(URG_POINTER_OFFSET)
    }

    /// Returns the TCP header options as an `[&u8]` slice. May panic if the value of `header_len`
    /// is invalid.
    #[inline]
    ...
    /// Parses TCP header options. It only supports the `MSS` option for now. If no error is
    /// encountered, returns the `MSS` value, or `None` if the option is not present. May panic if
    ...
    /// Interprets `bytes` as a TCP segment without any validity checks. May cause accessor
    /// methods to panic if `bytes` contains invalid input.
    #[inline]
...
impl<'a, T: NetworkBytesMut> TcpSegment<'a, T> {
    ...
    /// Sets the value of the byte containing every TCP flag except `NS`.
    #[inline]
    ...
    /// Returns a mutable slice containing the segment payload. May panic if the value of
    /// `header_len` is invalid.
    #[inline]
    ...
    /// and `checksum` fields. This function writes the rest of the segment, including data (when
    /// available). Since we only support the `MSS` option for now, we're not terribly generic in
    /// that regard. The `NS` flag, `URG` flag, and `urgent pointer` field are set to 0.
