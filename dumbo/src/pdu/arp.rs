...
pub enum Error {
    ...
    PLen,
    /// Invalid procotcol type.
    PType,
    /// The provided slice does not fit the size of a frame.
    SliceLength,
}

/// The inner bytes will be interpreted as an ARP frame. ARP is a generic protocol as far as data
/// link layer and network layer protocols go, but we're specifically interested in ARP frames
/// related to IPv4 over Ethernet.
...
impl<'a, T: NetworkBytes> EthIPv4ArpFrame<'a, T> {
    ...
    /// Futher method calls on the result may panic if `bytes` contains invalid input.
    #[inline]
    ...
    /// Tries to interpret a byte slice as a valid IPv4 over Ethernet ARP request. It guarantees we
    /// can safely call the accessor methods (which make use of various `_unchecked` functions),
    /// because all predefined offsets will be valid if this returns successfully.
