//! Defines traits which represent slices into sequences of bytes that stand for different values
//! packed together using network byte ordering (such as a network packet). Their main use is
//! reading and writing numerical values at a given offsets in the underlying slice.
//!
...
//!
//! What's with the `T: Deref<Target = [u8]>`? Do we really want/need to be that generic?
//! Not really. We actually only expect to work with byte slices (`&[u8]` and `&mut [u8]`),
//! but there's an annoying inconvenience we have to deal with. Let's say we define `NetworkBytes`
//! as:
//!
//! ```
...
//! trait bounds on `T`. For `NetworkBytes`, if `T` implements `Deref<Target = [u8]>` (which &[u8]
//! does), we define read operations. If `T` implements `DerefMut<Target = [u8]>`, we also define
//! write operations. Since `DerefMut<Target = [u8]>` implies `Deref<Target = [u8]>`,
...
//! we can extend this line of reasoning to structs which represent different kinds of protocol
//! data units (such as IPv4 packets, Ethernet frames, etc.) based on it.
//!
//! Finally, why `Deref` and not something like `AsRef`? The answer is `Deref` coercion, which in
//! our case means that a `NetworkBytes` value will automatically coerce to `&[u8]`
