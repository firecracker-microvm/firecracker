...
mod tests {
    ...
    fn test_hardcoded_snapshot_deserialization() {
        ...
        // level, by checking that only the version number and the newly added/removed fields
        // changes between versions are reflected in the hardcoded snapshot.
        ...
        let mut restored_struct: A =
            Snapshot::unchecked_load(&mut snapshot_blob, vm.clone()).unwrap();
