...
mod tests {
    ...
    impl io::Read for MockSerialInput {
        fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
            let count = unsafe { libc::read(self.0, buf.as_mut_ptr().cast::<c_void>(), buf.len()) };
            if count < 0 {
    ...
    fn test_issue_serial_hangup_anon_pipe_while_registered_stdin() {
        ...
        // FIFO read, or by the armed level-triggered stdin monitoring. Either one of the events
        // might be handled first. The handling of the second event will find the stdin
        // without any pending bytes and will result in EWOULDBLOCK. Usually, EWOULDBLOCK
        // will reregister the stdin, but since it was not unregistered before, it will do a
        // noop.
        ...
        // Process the kick stdin event generated by the reading of the 64th byte of the serial
        // FIFO. This will consume some more bytes from the stdin while the stdin is
        // unregistered.
        ev_count = event_manager.run().unwrap();
        assert_eq!(ev_count, 1);
    ...
    fn test_issue_serial_hangup_anon_pipe_while_unregistered_stdin() {
        ...
        // Process the kick stdin event generated by the reading of the 64th byte of the serial
        // FIFO. This will consume some more bytes from the stdin. Keep in mind that the
        // HANGUP event was lost and we do not know that the stdin reached EOF.
        ev_count = event_manager.run().unwrap();
        assert_eq!(ev_count, 1);

        // Two more bytes left. At the 2nd byte, another kick read stdin event is generated,
        // trying to fill again the serial FIFO with more bytes. Keep in mind that the HANGUP event
        // was lost and we do not know that the stdin reached EOF.
        for i in 0..2 {
            serial
                .lock()
                .unwrap()
